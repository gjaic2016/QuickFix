...
if (isset($_POST['_action_']) && $_POST['_action_'] == 'add_adds') {
	...
	$query .= " VALUES ('" . htmlspecialchars($_POST['title'], ENT_QUOTES) . "', '" . htmlspecialchars($_POST['description'], ENT_QUOTES) . "', '" . $_POST['archive'] . "', '" . date("Y-m-d H:i:s") . "')";
...
if (isset($_POST['_action_']) && $_POST['_action_'] == 'edit_adds') {
	$query  = "UPDATE adds SET title='" . htmlspecialchars($_POST['title'], ENT_QUOTES) . "', description='" . htmlspecialchars($_POST['description'], ENT_QUOTES) . "', archive='" . $_POST['archive'] . "'";
...
if (isset($_GET['delete']) && $_GET['delete'] != '') {
	...
	# Delete picture
	$query  = "SELECT picture FROM adds";
	$query .= " WHERE id=" . (int) $_GET['delete'] . " LIMIT 1";
	$result = @mysqli_query($MySQL, $query);
	$row = @mysqli_fetch_array($result);
	@unlink("adds/" . $row['picture']);

	# Delete adds
	$query  = "DELETE FROM adds";
...
#Edit adds
	...
	/*
		SELECT
			a.id, a.title, a.description, a.archive, a.date, u.username
		FROM
			adds a
		JOIN
			useradds ua ON a.id = ua.adds_id
		JOIN
	...
	*/
	...
	// $query  = "SELECT * FROM adds";
	$query = "SELECT a.id, a.title, a.description, a.archive, a.date, u.username FROM adds a JOIN useradds ua ON a.id = ua.adds_id JOIN users u ON u.id = ua.user_id";
	$query .= " ORDER BY date DESC";
	$result = @mysqli_query($MySQL, $query);

	// $query  = "SELECT username FROM users JOIN useradds on useradds.user_id = users.id";
	// $result = @mysqli_query($MySQL, $query);
